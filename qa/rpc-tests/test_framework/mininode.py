...
#
# NodeConn: an object which manages p2p connectivity to a neobytes node
...
def neobyteshash(s):
    return neobytes_hash.getPoWHash(s)
...
# Objects that map to neobytesd objects, which can be serialized/deserialized
...
class CBlockHeader(object):
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(neobyteshash(r))
            self.hash = encode(neobyteshash(r)[::-1], 'hex_codec').decode('ascii')
...
class msg_headers(object):
    ...
    def deserialize(self, f):
        # comment in neobytesd indicates these should be deserialized as blocks
